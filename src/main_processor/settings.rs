...
impl GameSettings {
    ...
    pub fn generate_backend(&self) -> Option<BackendInfo> {
...
//                    let dims = window.get_current_monitor().get_dimensions();
                ...
                WindowMode::Fullscreen => unimplemented!("Currently not implemented")
            }
        ...
        if window.get_inner_size().is_none() { None }
        else { Some(BackendInfo {
            event_loop,
            instance,
            surface
        }) }
